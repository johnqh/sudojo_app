 ...
 * @param pathname - URL pathname (e.g., "/en/daily" or "/daily")
 ...
 * @param language - The language code to prepend
 * @returns The path with language prefix
 */
export function addLanguageToPath(path: string, language: SupportedLanguage): string {
  const cleanPath = path.startsWith('/') ? path : `/${path}`;
  const existingLang = extractLanguageFromPath(cleanPath);
